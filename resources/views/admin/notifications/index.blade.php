                    ...
                    @foreach($notifications as $notification)
                        <div class="list-group-item list-group-item-action {{ $notification->read_at ? '' : 'unread-notification' }}">
                            <div class="d-flex w-100 justify-content-between align-items-center">
                                <div>
                                    @if($notification->data['for_admin'])
                                        <h5 class="mb-1">
                                            <i class="fas fa-shopping-cart text-primary me-2"></i>
                                            Nuevo Pedido: {{ $notification->data['order_number'] }}
                                        </h5>
                                        <p class="mb-1">Se ha recibido un nuevo pedido en la plataforma.</p>
                                    @else
                                        <h5 class="mb-1">
                                ...
                                <div class="d-flex">
                                    @if($notification->data['for_admin'] ?? false)
                                        <a href="{{ route('admin.orders.show', $notification->data['order_id']) }}" class="btn btn-sm btn-primary me-2">
